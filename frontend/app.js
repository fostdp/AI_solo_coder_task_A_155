(function () {
    ...
    function updateUI() {
        ...
        if (latestResonance) {
            document.getElementById('current-freq').textContent = `${latestResonance.measured_resonance_freq.toFixed(1)} Hz`;
            document.getElementById('theoretical-freq').textContent = `${latestResonance.theoretical_resonance_freq.toFixed(1)} Hz`;
            ...
            document.getElementById('gain-db').textContent = `${latestResonance.gain_db.toFixed(1)} dB`;
            document.getElementById('quality-factor').textContent = latestResonance.quality_factor.toFixed(1);
        }

        if (latestLocalization) {
            document.getElementById('bearing-angle').textContent = `${latestLocalization.bearing_angle.toFixed(1)} °`;
            document.getElementById('elevation-angle').textContent = `${latestLocalization.elevation_angle.toFixed(1)} °`;
            document.getElementById('distance-est').textContent = `${latestLocalization.distance_estimate.toFixed(0)} m`;
        ...
        if (Object.keys(sensorData).length > 0) {
            ...
            if (latest) {
                document.getElementById('spl-value').textContent = `${latest.sound_pressure_level.toFixed(1)} dB`;
                document.getElementById('density-value').textContent = `${latest.medium_density.toFixed(0)} kg/m³`;
    ...
    function initUI() {
        document.getElementById('btn-toggle-wave').onclick = function () {
        ...
        document.getElementById('btn-simulate-event').onclick = function () {
            simulateEnemyEvent();
        };
    }
    ...
    function handleWsMessage(msg) {
        switch (msg.message_type) {
            case 'sensor_data':
                ...
                spawnWave();
                updateDeviceGlow(msg.data.device_id, msg.data.sound_pressure_level);
                break;
            case 'resonance':
    ...
    function simulateEnemyEvent() {
        ...
        updateSourceMarker(latestLocalization);
        ...
        devices.forEach(d => {
            ...
            if (splHistory.length > MAX_SPL_HISTORY) splHistory.shift();
            spawnWave();
            updateDeviceGlow(d.device_id, reading.sound_pressure_level);
        });
